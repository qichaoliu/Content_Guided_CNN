...
for (curr_train_ratio,FLAG) in [(0.1,1)]:#(0.05,5),(0.01,4),(0.05,5),(0.01,2)
    ...
    for curr_seed in [0，1，2，3，4]:
        ...
        def coordinateXY(layer, kernel_size):
        ...
        def LocalPixels(input_feature, kernel_size, coordinateX,coordinateY, name='LocalPixels'):
            '''
            ...
            '''
            # x, y = coordinateXY(input_feature, kernel_size)
            ...
            with tf.variable_scope(name + "/_bilinear_interpolate"):
                ...
                # return f
                return tf.reshape(value_a, [num_batch,kernel_size[0] * height, kernel_size[1] * width,num_channels])

        def spatialsimilarity(input, kernel_size, sigma, coordinateX,coordinateY):
            localpixels = LocalPixels(input, kernel_size, coordinateX,coordinateY)
        ...
        def model(iter, last_layers_chanels, spatial_block_chanels):
            ...
            def SSConv(input, output_dims, similarity, coordinateX, coordinateY, name=None):
                a = BN(input)
                ...
                h_fc_localpixels = LocalPixels(b, (kernelsize, kernelsize), coordinateX,coordinateY)
            ...
            coordinateX,coordinateY = coordinateXY(input_img, (kernelsize, kernelsize))
            similarity = spatialsimilarity(spatialimg, [kernelsize, kernelsize], sigma, coordinateX,coordinateY)  # [1,145*5,145*5,1]

            # 第一层 由于提前进行BN化，因此第一层无需再BN
            h_conv1 = SSConv(input_img, last_layers_chanels, similarity, coordinateX, coordinateY, name='conv1')
            
            # 第二层
            h_conv2 = SSConv(h_conv1, spatial_block_chanels, similarity,coordinateX,coordinateY, name='conv2')
            h_conv2 = tf.concat([h_conv2, h_conv1], axis=-1)  # 拼接后的维度应该为203维
            
            # 第三层 spatial block
            h_conv3 = SSConv(h_conv2, spatial_block_chanels, similarity,coordinateX,coordinateY, name='conv3')
            h_conv3 = tf.concat([h_conv3, h_conv2], axis=-1)  # 拼接后的维度应该为448维
            
            # 第四层 spatial block
            h_conv4 = SSConv(h_conv3, spatial_block_chanels, similarity,coordinateX,coordinateY, name='conv4')
            h_conv4 = tf.concat([h_conv4, h_conv3], axis=-1)  # 拼接后的维度应该为480维
            
            # 第五层 spatial block
            h_conv5 = SSConv(h_conv4, spatial_block_chanels, similarity,coordinateX,coordinateY, name='conv5')
